...
- [Task #24a] ✅ 6 frequency-filtered channels (kick LPF 100Hz, snare BPF 200-2000Hz, hihat HPF 5000Hz, bass LPF 200Hz, melody BPF 300-4000Hz, bright HPF 3000Hz) each with their own FFT. 4 full-spectrum channels (perc, any, drop, soft) sharing main FFT via PV_Copy. Each channel has its own algorithm (mkl/phase/rcomplex/power), enable/disable toggle, and threshold config var. Removed old SC_ONSET_ALGORITHM and SC_ONSET_THRESHOLD. Added SC_FEATURE_ONSET_SUITE master toggle. Updated test mode to send synthetic triggers for all 10 channels at realistic rates. Updated OSCdef forwarders for all 10 onset paths (/audio/onset/kick, /snare, /hihat, /perc, /bass, /melody, /bright, /any, /drop, /soft).
