...
- [Task #45] ✅ — review found install scripts didn't copy sc/ directory. Fixed install.sh, INSTALL.md, INSTALL_WINDOWS.md.
- [Task #45] Split analyzer.scd (826 lines) into 5 files: analyzer.scd (97 lines, entry point), sc/config.scd (116 lines, env var reading), sc/synthdef.scd (297 lines, SynthDef), sc/osc_forwarding.scd (164 lines, OSCdefs + heartbeat), sc/test_mode.scd (162 lines, synthetic data). Uses ~ environment variables for shared state.
- [Task #44] ✅ — also removed dead beat-card flash CSS, vocal-bar CSS, and unused beatTimeout variable.
- [Task #44] Added Beat and Vocal as onset-style pips (12 total, 6x2 grid). Beat pip flashes via flashOnsetPip('beat'). Vocal pip lights when likelihood > 0.3. Removed vocal meter bar. Grid changed from 5 to 6 columns.
